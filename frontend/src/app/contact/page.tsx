        ...
        <div className="md:w-4/12 bg-blue-700 text-white p-10 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 pt-8 border-t border-white/20 mt-auto">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-blue-100">Lun - Sam</span>
              <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-bold">09h - 19h</span>
            </div>
          </div>
        </div>

        {/* COLONNE DROITE : FORMULAIRE (BLANC & SPACIEUX) */}
        <div className="md:w-8/12 p-8 md:p-12 bg-white overflow-y-auto">
          {isSuccess ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6 h-full flex flex-col justify-center">
              ...
              {/* MENU SUJET (CORRIGÉ : ABSOLUTE POSITION) */}
              ...
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-300 rounded-xl p-3 cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition-all text-center group">
                ...
                {fileNames.length > 0 && <div className="mt-2 flex flex-wrap justify-center gap-2">{fileNames.map((n, i) => <span key={i} className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-md font-medium truncate max-w-[150px]">{n}</span>)}</div>}
              </div>
