...
export default function Hero() {
  return (
    <section className="relative pt-32 pb-24 lg:pt-48 lg:pb-40 overflow-hidden">
      ...
      {/* --- FOND "PREMIUM" (DÉGRADÉ VIOLET/BLEU DOUX) --- */}
      <div className="absolute inset-0 -z-10 bg-white">
        {/* Halo Violet à gauche (Animation Blob) */}
        ...
        {/* Halo Bleu à droite (Animation Blob) */}
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-blue-200/30 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 mix-blend-multiply opacity-70 animate-blob animation-delay-2000" />
        {/* Texture légère pour la profondeur */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center relative z-10">
        ...
        {/* Badge "Disponible" (Style Sauvegarde) */}
        ...
        {/* Titre Impactant (Police sombre et grosse) */}
        ...
        {/* BOUTONS (Style Premium : Noir & Blanc) */}
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-20">
          <Link
            href="/contact"
            className="w-full sm:w-auto px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-full font-bold text-lg shadow-xl shadow-slate-200 hover:shadow-2xl transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2"
          >
            Demander un dépannage
          </Link>
          <Link
            href="/prestations"
            className="w-full sm:w-auto px-8 py-4 bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 rounded-full font-bold text-lg shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2"
          >
            Voir mes tarifs
          </Link>
        </div>

        {/* ARGUMENTS (Cartes Flottantes) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto text-left">
          {/* Carte 1 */}
          <div className="p-6 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 hover:border-blue-100 transition-colors">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              Votre ordinateur rame ? Je booste votre système Windows pour lui redonner sa vitesse d'origine.
            </p>
          </div>
          ...
          <div className="p-6 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 hover:border-blue-100 transition-colors">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              Suppression de virus, pages de pubs et piratage. Je sécurise vos données personnelles.
            </p>
          </div>
          ...
          <div className="p-6 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 hover:border-blue-100 transition-colors">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              Problème de connexion Internet ? J'installe votre Box, répéteur Wi-Fi et imprimante.
            </p>
          </div>
        </div>
