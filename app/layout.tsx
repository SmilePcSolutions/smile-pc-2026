...
export const metadata: Metadata = {
  title: "Smile PC Solutions | Dépannage Informatique 57",
  description: "Réparation PC, Mac à domicile. Intervention rapide Moyeuvre-Grande et alentours.",
};

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="fr" suppressHydrationWarning>
      <body className={[inter.className, "antialiased bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300"].join(" ")}>
        <ThemeProvider>
          <nav className="fixed top-0 w-full z-50 transition-colors duration-300 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800">
            <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
              ...
              <div className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600 dark:text-slate-300">
                ...
                <Link href="/prestations" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Prestations</Link>
                <Link href="/avis" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Avis Clients</Link>
                <Link href="/a-propos" className="hover:text-blue-600 dark:hover:text-blue-400 transition">À propos</Link>
                <div className="h-4 w-px bg-slate-200 dark:bg-slate-700 mx-2"></div>
                <ThemeToggle />
                <Link href="/contact" className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-full transition shadow-md hover:shadow-lg ml-2">
                  Me Contacter
                </Link>
              </div>
